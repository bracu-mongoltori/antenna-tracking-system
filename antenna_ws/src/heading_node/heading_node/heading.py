...
class QuaternionToEulerNode(Node):
    ...
    def orientation_callback(self, msg):
        ...
        self.roll_value.data = math.degrees(pitch)*(-1)
        self.pitch_value.data = math.degrees(yaw)*(-1)

        if self.roll_value.data < 0:
            self.roll_value.data += 360
            
        self.roll_value.data += 180.0
        self.roll_value.data = (self.roll_value.data % 360) 
        self.roll.publish(self.roll_value)
        print(f"roll: {self.roll_value.data}")

        if self.pitch_value.data < 0:
            self.pitch_value.data += 360
            
        self.pitch_value.data += 180.0
        self.pitch_value.data = (self.pitch_value.data % 360)
